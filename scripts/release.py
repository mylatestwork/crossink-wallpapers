...
def build_archive(filename: str, source_dir: Path, mode: str) -> Path:
    ...
    install = (
        ...
        f"Mode: {mode}\n"
        "Use this pack alone, or merge its .sleep contents with the other pack.\n"
        "Custom mode uses BMP files and ignores PNG files.\n"
        "Page Overlay mode can randomly select both BMP and PNG files.\n"
        "Copy the resulting .sleep folder to the root of the SD card.\n"
        "macOS: Press Command + Shift + . in Finder to show .sleep.\n"
        "Windows: .sleep is normally visible. If not, use View > Show > Hidden items.\n"
